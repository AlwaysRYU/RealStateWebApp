<template>
<div class="modal" tabindex="-1" id="detailModal" style="z-index: 2000">
  <div class="modal-dialog modal-dialog-centered modal-lg ">
    <div class="modal-content">
      <div class="modal-header bg-primary">
        <h5 class="modal-title" id="modal-title-color">글 상세</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <table class="table table-hover" >
          <tbody>
            <!--<tr><td>글번호</td><td>{{ $store.state.notice.boardId }}</td></tr>-->
            <tr><td>제목</td><td>{{ $store.state.notice.title }}</td></tr>
            <tr><td>작성자</td><td>{{ $store.state.notice.userName }}</td></tr>
            <tr id="tr-heigth"><td>내용</td><td v-html="$store.state.notice.content" id="solid-content"></td></tr>
      ...
      <div class="modal-footer">
        <button v-show="$store.state.notice.sameUser" @click="changeToUpdate" class="btn btn-sm btn-primary btn-outline" data-dismiss="modal" type="button">글 수정하기</button>
        <button v-show="$store.state.notice.sameUser" @click="changeToDelete" class="btn btn-sm btn-warning btn-outline" data-dismiss="modal" type="button">글 삭제하기</button>
      </div>
    </div>
  </div>
</div>
  ...
  //   },
    methods: {
      // http 요청 BoardMain 으로 이동!!
        ...
        //             this.sameUser = data.dto.sameUser;
        ...
        //         // this.sameUser = data.sameUser; // not data.dto.sameUser
        //       }
        //     })
        //     .catch((error) => {
