...
//C:\SSAFY_SpringBoot\Happy_House_5_Spring
    ...
    public BoardResultDto boardInsert(BoardDto dto, MultipartHttpServletRequest request) {// BoardController에서 파일을 넘겨 받음
	...
	public BoardResultDto boardDetail(BoardParamDto boardParamDto) {
	    ...
	    try {
	        ...
	        BoardDto boardDto = dao.boardDetail(boardParamDto); // 게시글을 읽어온다.
	...
	public BoardResultDto PopularBoards() {
	    ...
	    try {
	    	...
	    	// 1. 인기글
	    	BoardDto popularboard = dao.popularboard();
	    	System.out.println(popularboard);
	    	list.add(popularboard);
	    	
	        // 2. 공지시항
	    	BoardDto latestnotice = dao.latestnotice();
	    	list.add(latestnotice);
	...
	public BoardResultDto NoticeBoards() {
	    ...
	    try {
	        List<BoardDto> list = dao.boardLatestNotice();
